...
(define input '(3 6 4 8 1))   
(define output '(12 15 13 7 9))
...
(define (parallel-find-loss initial random-matrix-list)    
        (define ls1 0)
        (define ls2 0)
        (define ls3 0)
        (define ls4 0)
...
(define (looper initial n)
        ...
        (define loss (parallel-find-loss initial random-matrix-list))
...
(parallel-execute (lambda ()
  (let ((fin1 (looper initial 200)))
    (set! my-list (cons fin1 my-list))
    )) (lambda ()
  (let ((fin2 (looper initial 200)))
    (set! my-list (cons fin2 my-list))
    )) (lambda ()
  (let ((fin3 (looper initial 200)))
    (set! my-list (cons fin3 my-list))
    )) (lambda ()
  (let ((fin4 (looper initial 200)))
    (set! my-list (cons fin4 my-list))
    )) )

(define finalsum (add-list-of-matrices my-list))
(define finalavg (divide-matrix-by-number finalsum 4))
(display (multiplywithinput finalavg))
